...
//  GHFollowers
//
//  Created by Khant Zaw Win on 14/10/21.
//
        ...
        Task{
            do {
                ...
                if let gfError = error as? GFError {
                    presentGFAlert(title: "Badd Stuff", message: gfError.rawValue, buttonTitle: "Ok")
                } else {
                    presentDefaultError()
                }
                dismissLoadingView()
...
//                self.addUserToFavourite(user: user)
//                break
//            case .failure(let error):
...
//        }
    ...
    func addUserToFavourite(user: User){
        ...
        PersistenceManager.updateWith(favoirte: favorite, actionType: .add) {[weak self] error in
            guard let self = self else {return}
...
extension FollowerListVC: UserInfoVCDelegate {
    func didRequesetFollowers(for username: String) {
        self.username = username
        title = username
        page = 1
